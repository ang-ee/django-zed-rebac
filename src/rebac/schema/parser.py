...
_BACKING_DIRECTIVE_RE = re.compile(r"//\s*rebac:(field|const|attribute)\s*=\s*(.*?)\s*$")
...
class _Parser:
    def __init__(self, text: str):
        ...
        self.pos = 0
        self.backing_directives_by_line = self._extract_backing_directives()
    ...
    def _extract_backing_directives(
        self,
    ) -> dict[int, FieldBinding | ConstBinding | AttributeBinding]:
        directives: dict[int, FieldBinding | ConstBinding | AttributeBinding] = {}
        for lineno, line in enumerate(self.text.splitlines(), start=1):
            comment_start = line.find("//")
            if comment_start == -1:
                continue
            comment = line[comment_start:].strip()
            match = _BACKING_DIRECTIVE_RE.fullmatch(comment)
            if match is None:
                if re.match(r"//\s*rebac:(field|const|attribute)\b", comment):
                    raise ParseError(f"Malformed relation backing directive at line {lineno}")
                continue
            ...
            if backing is not None:
                directives[lineno] = backing
        return directives

    def _backing_between(
        self, *, start_line: int, end_line: int
    ) -> FieldBinding | ConstBinding | AttributeBinding | None:
        matches = [
            (line, backing)
            for line, backing in self.backing_directives_by_line.items()
            if start_line <= line <= end_line
        ]
        if len(matches) > 1:
            lines = ", ".join(str(line) for line, _ in matches)
            raise ParseError(f"Multiple backing directives on relation at lines {lines}")
        ...
        for relation in definition.relations:
            ...
            if relation.backing is not None:
                try:
                    ...
                    errors.append(f"{definition.resource_type}#{relation.name}: {exc}")
