...
from rebac.backends import LocalBackend, backend as active_backend, reset_backend
...
def test_default_subject_relation_must_be_a_relation_on_effective_type(
    db, install_schema
):
    install_schema(
        """
...
def test_default_subject_relation_accepts_declared_relation(db, install_schema):
    ...
    assert not any(
        issue.id == "rebac.E011" and "blog/subjectcontainer" in issue.msg
        for issue in issues
    )
...
def test_attribute_backing_model_errors_are_reported_as_e009(db, install_schema):
    ...
    assert any(
        issue.id == "rebac.E009" and "missing attribute 'missing'" in issue.msg
        for issue in issues
    )
